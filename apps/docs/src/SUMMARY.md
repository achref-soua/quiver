# Summary

[Introduction](introduction.md)

# Getting started

- [Concepts](concepts.md)
- [Quickstart](quickstart.md)
- [Self-hosting & configuration](self-hosting.md)

# Guides

- [RAG with Quiver](guides/rag.md)
- [Agentic patterns (MCP)](guides/agentic.md)
- [Tuning for RAG](guides/tuning.md)

# Features

- [Indexing & memory frugality](features/indexing.md)
- [Hybrid (dense + sparse) search](features/hybrid-search.md)
- [Server-side embedding & reranking](features/embedding.md)
- [Multi-vector / late interaction](features/multi-vector.md)
- [Encrypted vector search](features/encrypted-search.md)
- [Migration importers](features/migration.md)
- [Replication](features/replication.md)
- [Snapshots & backup](features/snapshot.md)
- [Observability (metrics & tracing)](features/observability.md)

# API & SDKs

- [REST & gRPC](api/rest-grpc.md)
- [MCP server](api/mcp.md)
- [Python, TypeScript & Go SDKs](api/sdks.md)

# Security

- [Cryptography & posture](security/overview.md)
- [DCPE encrypted search](security/dcpe.md)
- [Client-side opaque vectors](security/client-side-vectors.md)
- [Threat model](security/threat-model.md)

# Architecture

- [Deep dive](architecture/deep-dive.md)
- [Decision records (ADRs)](architecture/adrs.md)
