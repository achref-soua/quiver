...
Search your embeddings on a server that **never sees the plaintext vectors or the key**. With the opt-in, off-by-default `encrypted_vectors` collection flag (ADR-0031), the client encrypts vectors with **distance-comparison-preserving encryption** — the published [Scale-And-Perturb scheme](https://eprint.iacr.org/2021/1666), built only from audited RustCrypto primitives — so the server can rank ciphertexts by approximate L2 distance without decrypting them. Ships as a Rust reference (`quiver_crypto::dcpe`), a native Python `DcpeCipher`, and the flag across REST/gRPC/MCP/SDKs.

This is **experimental and not semantically secure**: it is L2-only and **leaks the approximate distance-comparison relation by design** (that is how the server ranks), so it carries real, documented caveats and is broken by known-plaintext or strong-prior adversaries. It **complements** encryption-at-rest; it does not replace it. Read [`docs/security/dcpe.md`](./docs/security/dcpe.md) before using it.
