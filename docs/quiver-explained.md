...
> 💡 **What this means, without spin.** The `O(n)` RAM walls that used to cap a modest box at ~10M are **closed** — 100M is now functional-by-design on frugal hardware, not gated on unwritten work. What is **not** yet claimed is the end-to-end **RSS at ≥20M/100M**: that measurement is deferred to a dedicated, idle box and will be published when it is run — never fabricated, never extrapolated. The one named remaining build-time ceiling is the non-MVCC incremental IVF rebuild tail.
