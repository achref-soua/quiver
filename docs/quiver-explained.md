...
What is *not* yet built is deliberately so, until the single-node story is unbeatable and the need is concrete. The big features now stand: write HA (§9.11), autoscaling (§9.12), and the GPU distance kernel (§9.13). Safe scale-*in* has since been ticked off that list: the reverse-migration **drain** (§9.12, [ADR-0080](./adr/0080-online-scale-in-drain.md)) is now one call, so elastic scaling runs in both directions. What remains is mostly **wiring and evidence** — threading the GPU kernel through more of the search and build paths (where it plugs in, and the contract it must honour, are now settled in [ADR-0079](./adr/0079-gpu-build-search-wiring.md); the wiring itself waits on real GPU hardware), and the launch polish a `v1.0.0` is gated on: the published **reference-hardware** benchmark table (recorded on dedicated machines, never fabricated) and a recorded cockpit cast. One of that polish list is now ticked off: the **hosted docs site is live** at [`achref-soua.github.io/quiver`](https://achref-soua.github.io/quiver/) — the `apps/docs` mdBook, which reuses these same canonical documents so there is one source of truth, published to GitHub Pages straight from the release ([ADR-0078](./adr/0078-docs-site-deploy.md)). None of this compromises the single static binary, and single-node remains the default everywhere.
