...
### The honest ceiling: 10M measured here, 100M now unblocked in code — RSS still to be measured

This box measures a full index build to **10M** vectors. The path to 100M on a modest box was gated by three `O(n)` **resident** terms; all three have since been moved off-heap, each in its own release:

- **The build's `n × dim` float arena is gone** ([ADR-0071](./adr/0071-streaming-rebuild-vector-source.md), v0.33.0). The IVF rebuild used to materialize every vector in a resident `flat` buffer (`scan_collection`) — ~5 GiB at 10M, a projected **~51 GiB at 100M**. It now **streams vectors from the immutable on-disk `.vec` segments**, sample-trains the codebooks, and stream-encodes, so peak build memory for the vectors drops from `O(n·dim)` to `O(sample + nlist·dim + n·m_bytes)` — the corpus stays on disk, read on demand.
- **The primary index left RAM** ([ADR-0072](./adr/0072-on-disk-primary-index.md), v0.34.0). The per-collection `ext_id → location` map was **~316 B/point** (a projected ~31 GiB at 100M) fully resident. It is now served from the sealed segments' on-disk forward index, dropping resident state to **~1.6 GiB at 100M** (per-segment byte offsets, forward index, and tombstone bitmap only).
- **The embedded index's id map left RAM** ([ADR-0073](./adr/0073-on-disk-embed-id-map.md), v0.35.0). The ANN index still held `int_to_ext` + `ext_to_int` — the external ids twice over, ~90–110 B/point (~9–11 GiB at 100M) — and re-serialized them on every checkpoint. The base ids now live in an on-demand-paged `ExtIdColumn`, so resident id state drops to **~12 B/point** and the checkpoint envelope no longer carries them.

Closing those RAM walls left one **wall-clock** gate. A trial 100M ingest (2026-07-21) confirmed the resident state stays small — but that moving the primary index to disk (ADR-0072) made every write's existence check an `O(segments · log rows)` **decrypt-on-compare** probe: to know whether an id is already stored, `locate` binary-searches each sealed segment's on-disk id index, decrypting a candidate id on every comparison. Single-threaded ingest therefore slowed as sealed segments accumulated — RAM-feasible, but wall-clock-prohibitive at 100M.

- **The per-write existence probe is pruned** ([ADR-0076](./adr/0076-resident-id-range-fingerprint.md), the upgrade path ADR-0072 named). Each sealed segment now keeps a resident **min/max external-id range** — two strings per segment, derived on open from the id-sorted forward index, so **no on-disk format change and no per-point RAM**. `lookup` range-rejects an out-of-range id with one string compare before the decrypting search. For sortable ids (sequences/ULIDs/timestamps — the realistic large-N shape) every new id sorts beyond every sealed segment's max, so the probe collapses to `O(segments)` compares with zero decrypts. Measured on the scale harness (1M vectors, IVF+PQ m=16, sealing every 100k rows): **ingest 113.7 s → 18.7 s (~6.1×), peak RSS 567 → 568 MiB (unchanged)**. That is what makes the on-laptop 100M run wall-clock-practical.

> 💡 **What this means, without spin.** The `O(n)` RAM walls that used to cap a modest box at ~10M are **closed**, and the ingest slowdown that would have made a 100M run take hours is **pruned** — 100M is now functional-by-design *and* wall-clock-practical on frugal hardware, not gated on unwritten work. What is **not** yet claimed is the end-to-end **RSS and wall-clock at ≥20M/100M**: that measurement is deferred to a dedicated, idle box and will be published when it is run — never fabricated, never extrapolated. The one named remaining build-time ceiling is the non-MVCC incremental IVF rebuild tail.
...
That round-trip is where a **bulk** load could hurt — thousands of books filed in one request. The naïve way waits for the quorum to acknowledge each book before proposing the next, so a thousand books pay a thousand round-trips back to back, and throughput collapses to one-per-round-trip — the classic un-pipelined mistake. Quiver instead **pipelines** the proposals ([ADR-0077](./adr/0077-batched-raft-proposals.md)): it hands the whole batch to the leader at once, up to a bounded number in flight, and the consensus library coalesces the pending entries into a *single* log append (one `fsync`) and a *single* replication round. The books still commit in the exact order they were handed over — so the result is identical, and each one is still individually quorum-committed and durable — but the *waiting overlaps* instead of stacking. On a real multi-node cluster, where that round-trip is a network hop, this is the difference between a bulk import that crawls and one that flies. Honestly, on a single box there is no network latency to overlap and the path is disk-bound, so the measured gain there is slight (~1.1×); the real win is a cluster property, and the cluster number waits for real multi-node hardware rather than being guessed.
...
In Quiver terms: set an `[autoscale]` table on the coordinator (`enabled`, `high_water_points`, a `standby_urls` list, `interval_secs` / `cooldown_secs` / `max_shards`) and it grows the cluster from the standby pool on its own. Off by default; a cluster without the policy is exactly the one §9.10 describes.

**Closing a room is now one call too.** The reverse of §9.10 used to be the manual part: you drained a shard by hand and then deleted it, because deleting one that still held books would strand them. `POST /cluster/shards/{id}/drain` does the whole thing online ([ADR-0080](./adr/0080-online-scale-in-drain.md)) — and it is the mirror image of the join, down to the mechanism. A room marked **leaving** hands its books' *ownership* to its neighbours instantly (one map version, and to exactly the neighbours that will own them once it is gone), but it keeps the books on its own shelves and keeps answering for them until the copy finishes — so every write during the drain is filed in both places, and the room is still searched. Then it leaves the map, and only then are its now-redundant copies removed — each one checked against its new home first, so a copy is never deleted unless someone else demonstrably has it. Same two promises as growing: nothing acknowledged is lost, and the slice answers queries the whole time. Abort halfway and nothing is lost either, because the leaving room was kept current throughout.

What is still deliberately *not* automatic is the **decision** to shrink. Growing on a busy signal is safe; shrinking on a quiet one invites flapping — drain a machine out, refill it, drain it again — and unlike a grow it decommissions hardware. The dangerous part was the operation, and the operation is now safe; the policy stays yours.
...
The catch is that most self-hosted boxes **don't** have one, and a hard CUDA dependency would wreck the "single static binary that runs anywhere" promise. So the GPU lives entirely behind a seam (the distance kernel, §4.2) and an **off-by-default `cuda` build feature**, exactly like the `raft` and `otlp` features: a normal build links **zero** CUDA, the CPU SIMD kernel stays the default *and* the fallback, and the GPU produces **identical** numbers — it is a pure accelerator, never a correctness dependency, and it never touches the on-disk format or the crash gate. The CUDA driver is even loaded *dynamically* and the kernel compiled at runtime, so the feature **compiles without a CUDA toolchain** — only *running* the GPU path needs a device, and if there is no device the code falls back to the CPU.

That last sentence was, for several releases, simply false — and it is worth knowing why, because it is the kind of bug that hides behind a reassuring comment. The fallback was written as ordinary error handling: try to open a device, and on failure take the CPU path. But the CUDA driver is loaded by a library that **panics** when it cannot find `libcuda` rather than returning an error, so the failure unwound straight past the code meant to catch it. On any machine without CUDA installed — the common case, and precisely the case the fallback exists for — a `cuda` build did not quietly use the CPU; it crashed. The test that caught it was the one asserting the *CPU* result. It is fixed ([ADR-0052](./adr/0052-gpu-acceleration.md) seam, `init_or_none`), the fallback is now tested on every CI run on a machine that deliberately has no GPU, and the lesson generalises: a fallback that is never exercised is not a fallback, it is a comment.
