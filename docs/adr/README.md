...
| [0065](0065-cluster-mode-implementation.md) | Cluster mode — implementation design: takes ADR-0051 from design-only to built; opt-in, sharding + scatter-gather first, per-shard consensus via an **audited Raft crate** (not hand-rolled) last; single-node stays the zero-overhead default | Accepted | 4 |
