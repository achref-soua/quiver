...
- **Honest deviations from the decision above:** the AEAD is **XChaCha20-Poly1305** throughout (not AES-256-GCM with AES-NI auto-select) — its extended random nonce removes the nonce-reuse footgun without a counter, and it matches the at-rest codec; AES-NI selection is a future option. DEK wrapping uses **XChaCha20-Poly1305** (not AES-256-GCM-SIV) for the same reason and to avoid an extra primitive.
- **Master-key source (shipped):** the MK is supplied via `QUIVER_ENCRYPTION_KEY` (hex) **or** `QUIVER_MASTER_KEY_FILE` (a file holding the hex), exactly one of the two; the file form lets the key arrive as a mounted Docker/Kubernetes secret or a KMS-decrypted file rather than an environment variable, and a group/world-readable file is warned about at startup. A built-in KMS client (decrypt the wrapped MK via an external KMS at boot) is a documented future step in front of the file source, not yet implemented.
