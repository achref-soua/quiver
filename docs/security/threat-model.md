...
> **Client-side payload encryption protects payloads, not vectors.** Confidentiality of *vectors* against the server is **not** provided by default. It is addressed only by the **experimental** distance-comparison-preserving encryption (DCPE) track — a *published* scheme behind a feature flag, which by design **leaks approximate distance/ordering** (that is how the server can still rank) and therefore carries real, documented leakage caveats. Quiver does **not** claim homomorphic-encrypted search in core, and never ships a home-grown scheme.
